#!/usr/bin/env bash
# Lab IdP stub — key material + JWKS for the `jwks-stub` compose service (ADR-0524 JWT verification).
# Pattern mirrors docker/mtls/generate-certs.sh: this script is versioned; everything it writes is
# git-ignored and regenerable on demand. NEVER commit the private key.
#
...
#
# The services' decoders (NimbusJwtDecoder.withJwkSetUri — SecurityConfig in all three services)
# match tokens by kid, so the kid here and in minted tokens must agree; mint-jwt.sh reads it back
# from public/jwks.json rather than duplicating the constant.
set -euo pipefail
cd "$(dirname "$0")"

KID="${JWKS_KID:-chaosforge-lab-1}"
...
echo "serve it:    docker compose -f ../../docker-compose_chaosforge.yml up -d jwks-stub"
echo "mint tokens: ./mint-jwt.sh --help"
