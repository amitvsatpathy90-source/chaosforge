...
/**
 * Outbox producer: String key (partition key = scenario_id, ADR-0526), Avro-binary byte[] value.
 * Idempotent + acks=all so a producer retry never duplicates a broker write.
 */
@Configuration
public class KafkaProducerConfig {
    ...
    public KafkaTemplate<String, byte[]> outboxKafkaTemplate(ProducerFactory<String, byte[]> outboxProducerFactory) {
        KafkaTemplate<String, byte[]> template = new KafkaTemplate<>(outboxProducerFactory);
        // Injects W3C traceparent headers so CP-relay and exec-pipeline logs share a trace_id.
        // This is the relay's trace, not the original HTTP request's — the outbox row severs that.
