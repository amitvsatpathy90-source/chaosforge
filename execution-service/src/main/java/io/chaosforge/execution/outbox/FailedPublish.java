 ...
 * under the relay-lag alerts — a broker outage must never mass-DEAD the outbox and force manual
 * data-repair writes (the C31 bar).
 *
 * <p>{@code error} is an exception-class summary only — never payload bytes (PII rule).
 */
record FailedPublish(ExecOutboxRecord row, String error, boolean recordFatal) {
}
