 ...
 * Crash between claim/finalize leaves the row PENDING for lease retry; claimed_by tracks takeovers.
 */
@Component
public class ExecOutboxRelayDao {

    // Hot lane: atomic claim + lease. Public so ExecOutboxClaimLanesIT can EXPLAIN the real statement.
    public static final String CLAIM_SQL = """
            ...
            RETURNING o.message_id, o.msg_ts, o.tenant_id, o.topic, o.partition_key, o.replay_version,
                      o.payload, o.attempts, due.claimed_by AS prior_claimed_by""";
    ...
    private static final RowMapper<ExecOutboxRecord> MAPPER = (rs, n) -> new ExecOutboxRecord(
    ...
    public ExecOutboxRelayDao(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    /** Hot-lane claim: due rows younger than the window. Run inside its own short transaction. */
    public List<ExecOutboxRecord> claimAndLease(int batchSize, int leaseSeconds, int claimWindowDays,
                                                String instanceId) {
    ...
    public List<ExecOutboxRecord> claimAndLeaseStragglers(int batchSize, int leaseSeconds,
                                                          int claimWindowDays, String instanceId) {
    ...
    public void markSentBatch(List<ExecOutboxRecord> sent) {
        if (sent.isEmpty()) {
            return;
        }
        jdbc.batchUpdate(
