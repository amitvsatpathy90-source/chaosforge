...
import io.chaosforge.execution.outbox.ExecOutboxRecord;
...
/**
 * The G1 reconciliation on the Execution Service result-event outbox, proven against real
 * partitioned Postgres (V8 range partitioning, V10 claim attribution). Mirrors the CP
 * {@code OutboxClaimLanesIT} — the relays are deliberately independent code, so each service carries
 * its own regression proof:
 *
 * <ul>
 *   <li><b>Plan shape (G1/A-1):</b> the hot-lane claim touches {@code outbox} through index paths
 *       only — no partition Seq Scan (the self-join carries the full {@code (msg_ts, message_id)}
 *       PK), no Sort node. The legacy {@code message_id IN (subquery)} shape is the negative
 *       control: the same detector must flag it, or these assertions are vacuous;</li>
 *   <li><b>Lane complementarity (B-2):</b> a row older than the hot window is claimable by the
 *       straggler lane — no PENDING row is unreachable before its partition drops;</li>
 *   <li><b>Claim attribution (B-4):</b> a re-claim of another instance's leased row exposes the
 *       prior claimant.</li>
 * </ul>
 */
class ExecOutboxClaimLanesIT extends ExecPostgresIT {

    /** The pre-V10 claim shape (verbatim), kept as the negative control — see the CP twin. */
    ...
    void hotClaimPlan_indexPathsOnly_noSeqScan_noSortNode() {
        ...
        String plan = explain(ExecOutboxRelayDao.CLAIM_SQL, 2, 64, 30, "plan-probe");
        ...
        assertThat(plan)
                .as("ORDER BY next_attempt_at rides idx_exec_outbox_due in index order — no Sort of the due set\n%s", plan)
                .doesNotContain("->  Sort")
    ...
    void legacyClaimShape_negativeControl_plannerNeedsSeqScanAndSort() {
        ...
        // defects the assertions above forbid.
        seedBulkPending(150_000);
        String populated = mostPopulatedPartition();
        ...
        assertThat(plan)
                .as("ORDER BY message_id cannot ride idx_exec_outbox_due — the whole due set is sorted\n%s", plan)
                .contains("->  Sort");
    }
    ...
    void hotLaneSkipsOutsideWindowRow_stragglerLaneClaimsIt() {
        ...
        ExecOutboxRelayDao dao = new ExecOutboxRelayDao(jdbc);

        List<ExecOutboxRecord> hot = tx.execute(s -> dao.claimAndLease(64, 30, 2, "hot-lane"));
        assertThat(hot).extracting(ExecOutboxRecord::messageId)
                .as("hot lane sees only rows inside the claim window").containsExactly(fresh);

        List<ExecOutboxRecord> cold = tx.execute(s -> dao.claimAndLeaseStragglers(64, 30, 2, "cold-lane"));
        assertThat(cold).extracting(ExecOutboxRecord::messageId)
    ...
    void reclaimByAnotherInstance_exposesPriorClaimant() {
        ...
        List<ExecOutboxRecord> first = tx.execute(s -> dao.claimAndLease(64, 30, 2, "relay-A"));
        ...
        List<ExecOutboxRecord> second = tx.execute(s -> dao.claimAndLease(64, 30, 2, "relay-B"));
    ...
    private void seedBulkPending(int rows) {
        jdbc.update("""
                INSERT INTO outbox (message_id, msg_ts, aggregate_id, tenant_id, topic, partition_key,
                                    replay_version, payload)
                SELECT gen_random_uuid(),
                       now() - make_interval(days => CASE WHEN i % 10 = 0 THEN 3
                                                          WHEN i % 10 = 1 THEN 5 ELSE 0 END),
                       gen_random_uuid(), gen_random_uuid(), 'chaosforge.scenario.results.v1',
                       i::text, 1, '\\x01'::bytea
    ...
    private UUID insertPendingAgedDays(int agedDays) {
        ...
        jdbc.update("INSERT INTO outbox (message_id, msg_ts, aggregate_id, tenant_id, topic, "
                        + "partition_key, replay_version, payload) "
                        + "VALUES (?, now() - make_interval(days => ?), ?, ?, ?, ?, ?, ?)",
                id, agedDays, UUID.randomUUID(), UUID.randomUUID(), "chaosforge.scenario.results.v1",
                id.toString(), 1L, new byte[] {0x1});
        return id;
    }
}
